...
public interface UserDao {
    ...
    @Select("SELECT * FROM userinfo WHERE username=#{username} AND password=Password(#{password}) AND pid=#{pid}")
    ...
    //添加用户
    @Insert("INSERT INTO userinfo (username,password,pid,memorandum) VALUE (#{username},Password(#{password}),#{pid},#{memorandum})")
    ...
    //查询输入正确用户名密码的用户身份
    @Select("SELECT pid FROM userinfo WHERE username=#{username} AND password=Password(#{password})")
    String getPid(@Param("username") String username, @Param("password") String password);

    //根据用户名更新用户
    @Update("UPDATE userinfo SET password=Password(#{password}) ,memorandum=#{memorandum} WHERE username=#{username}")
