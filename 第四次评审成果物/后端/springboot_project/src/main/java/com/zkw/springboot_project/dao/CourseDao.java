...
public interface CourseDao {
    ...
    //更改课程发布状态
    @Update("UPDATE course_info1 SET state=#{state} WHERE courseid = #{courseid}")
    int editState(@Param("state") String state,@Param("courseid") String courseid);

    //----------------------------------公众--------------------------------------------
    @Select("SELECT courseid,username,cname,points,period,theory,practice,major,attribute FROM course_info1 WHERE courseid like #{courseid} AND state like 1 LIMIT #{pageStart},#{pageSize}")
    List<Course> getAllPublicCourseBase(@Param("courseid") String courseid,@Param("pageStart") int pageStart, @Param("pageSize") int pageSize);

    //获取该课程负责人负责的课程条目数
    @Select("SELECT count(*) FROM course_info1 WHERE courseid like #{courseid} AND state like 1")
    int getPublicCourseCounts(@Param("courseid") String courseid);
